...
class ProgramControllerIT extends IntegrationTest {
    ...
    @Sql("/sql/hr/program-draft-test.sql")
    @WithMockUser(username = "hr_prog", authorities = "HR")
    void getCompanyUsers_filtersByManagerRole() throws Exception {
        // fixture: hr_prog has HR+USER, user1/user2 are plain USER, none are MANAGER → empty list
        var result = mvc.perform(get("/api/latest/hr/programs/users").param("role", "MANAGER"))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    @Sql("/sql/hr/program-draft-test.sql")
    @WithMockUser(username = "hr_prog", authorities = "HR")
    void getCompanyUsers_filtersByHrRole() throws Exception {
        var result = mvc.perform(get("/api/latest/hr/programs/users").param("role", "HR"))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    @Sql("/sql/hr/program-draft-test.sql")
    @WithMockUser(username = "hr_prog", authorities = "HR")
    void getCompanyUsers_defaultsToUserRole_whenRoleOmitted() throws Exception {
        // Unchanged default behavior: all users with USER role (incl. hr_prog which has ["HR","USER"])
        var result = mvc.perform(get("/api/latest/hr/programs/users"))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        ...
        TestUtils.assertJsonEquals(result, """
                [
                    {"username": "hr_prog"},
                    {"username": "user1@test.cz"},
                    {"username": "user2@test.cz"}
                ]
                """);
    }
