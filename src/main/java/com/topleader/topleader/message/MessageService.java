...
public class MessageService {
    ...
    public List<ChatInfoDto> getUserChatInfo(String username) {
        ...
        final var unreadCountMap = getUnreadMessagesCount(username).stream()
        ...
        return allChats.entrySet().stream()
            ...
            .map(e -> new ChatInfoDto(
                ...
                lastMessages.get(e.getValue()).messageData(),
                lastMessages.get(e.getValue()).createdAt().atZone(userZoneId)
    ...
    public void sendMessage(String username, String addressee, String messageData) {
        ...
        final var message = messageRepository.save(
            new Message(null, chat.getChatId(), username, addressee, messageData, false, time, false)
        );

        lastMessageRepository.upsert(chat.getChatId(), message.id());
    ...
    public List<UnreadMessagesCount> getUnreadMessagesCount(String username) {
        return jdbc.query(
                "SELECT user_from AS userfrom, COUNT(*) AS unread FROM user_message WHERE user_to = :username AND displayed = false GROUP BY user_from",
                Map.of("username", username),
                (rs, rowNum) -> new UnreadMessagesCount(rs.getString("userfrom"), rs.getLong("unread"))
        );
    }
