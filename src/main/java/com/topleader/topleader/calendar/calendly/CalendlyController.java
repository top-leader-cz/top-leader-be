...
public class CalendlyController {
    ...
    public RedirectView cal(String code) {
        ...
        try {
            ...
            return new RedirectView("/#/sync-failure?provider=calendly&error=sync.failed");
        }
        ...
        return Try.run(() -> calendlyService.saveInfo(info))
                ...
                .getOrElse(new RedirectView("/#/sync-failure?provider=calendly&error=user.email.not.matched"));
    }
}
