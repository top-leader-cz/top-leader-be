...
public class AiClient {
    ...
    // Ranks coaches by relevance and returns top N with reasons.
    // Used by coach-preview to fill/trim pool to target size.
    public List<CoachRecommendation> rankCoaches(String programGoal, List<String> focusAreas, String targetGroup, List<CoachProfile> coaches, int topN) {
        log.info("Ranking {} coaches for program goal: {}, focusAreas: {}, topN: {}", coaches.size(), programGoal, focusAreas, topN);
        ...
        var systemPrompt = """
                You are an expert at matching coaches to corporate programs.
                Given a program's goal, focus areas, and target group, rank ALL coaches
                from the provided list by relevance and return the top %d best matches.

                Evaluate coaches based on:
                - Bio and coaching philosophy alignment with program goal
                - Fields and topics matching program focus areas
                - Primary roles (COACH, MENTOR, TRAINER) relevance
                - Priority score (higher = more preferred by the platform)

                Return a JSON array of up to %d objects with fields: username, firstName, lastName, reason.
                The reason should be 1-2 sentences explaining why this coach is a good fit.
                Order by best match first. Return ONLY the JSON array, no other text.
                If fewer than %d coaches are relevant, return fewer.
                """.formatted(topN, topN, topN);
