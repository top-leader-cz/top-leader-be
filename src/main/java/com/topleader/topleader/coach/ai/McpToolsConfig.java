package com.topleader.topleader.coach.ai;
...
public class McpToolsConfig {
    ...
    public Function<AiClient.TavilySearchRequest, List<AiClient.TavilySearchResult>> searchArticles() {
    ...
    public Function<AiClient.TavilySearchRequest, List<AiClient.TavilySearchResult>> searchVideos() {
        return request -> tavilySearch(request.query(), List.of("youtube.com"), 5);
    }

    private List<AiClient.TavilySearchResult> tavilySearch(String query, List<String> includeDomains, int maxResults) {
        ...
        var items = Optional.ofNullable(results).orElse(List.of()).stream()
                .map(r -> new AiClient.TavilySearchResult(
                        (String) r.get("title"),
                        (String) r.get("url"),
